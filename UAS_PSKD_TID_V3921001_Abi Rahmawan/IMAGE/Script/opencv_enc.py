...
demo = cv2.imread("D:/SKD/image.jpg", 0)
r, c = demo.shape
key = np.random.randint(0, 256, size=(r, c), dtype=np.uint8)  # Generate random key image
cv2.imwrite("D:/SKD/key.jpeg", key)   # Save key image
...
cv2.imwrite("D:/SKD/image_enkripsi.jpg", encryption)  
   # Save the encrypted image
decryption = cv2.bitwise_xor(encryption, key)  # decrypt
cv2.imwrite("D:/SKD/image_dekripsi.jpg", decryption) # Save the decrypted image
